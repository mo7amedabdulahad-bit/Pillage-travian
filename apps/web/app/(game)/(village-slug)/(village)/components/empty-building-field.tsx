...
const getTribeColors = (tribeValue: Tribe): { default: string; hover: string } => {
  const colorMap: Record<Tribe, { default: string; hover: string }> = {
    teutons: { default: 'rgba(44, 58, 11, 0.57)', hover: 'rgba(85, 96, 39, 0.57)' },
    romans: { default: 'rgba(60, 79, 86, 0.35)', hover: 'rgba(60, 79, 86, 0.23)' },
    gauls: { default: 'rgba(101, 115, 13, 0.57)', hover: 'rgba(206, 223, 100, 0.57)' },
    huns: { default: 'rgba(44, 58, 11, 0.57)', hover: 'rgba(85, 96, 39, 0.57)' }, // Defaulting to Teuton for now
    egyptians: { default: 'rgba(60, 79, 86, 0.35)', hover: 'rgba(60, 79, 86, 0.23)' }, // Defaulting to Roman for now
    spartans: { default: 'rgba(44, 58, 11, 0.57)', hover: 'rgba(85, 96, 39, 0.57)' },
    nature: { default: 'rgba(60, 79, 86, 0.35)', hover: 'rgba(60, 79, 86, 0.23)' },
    natars: { default: 'rgba(44, 58, 11, 0.57)', hover: 'rgba(85, 96, 39, 0.57)' },
  };
...
export const EmptyBuildingField = ({
  ...
  return (
    <Link
      ...
      data-name=""
      className={clsx(
        'building-slot g0 emptyBuildingSlot block w-full h-full cursor-pointer focus:outline-hidden',
        `a${buildingFieldId}`,
        `aid${buildingFieldId}`,
        getTribeClass(tribe),
      )}
    >
      ...
      >
        <path
          ...
          onMouseEnter={(e) => {
            (e.currentTarget as SVGPathElement).style.fill = colors.hover;
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as SVGPathElement).style.fill = colors.default;
          }}
        />
      </svg>
    </Link>
