...
import {
  getResourceFieldImagePath,
  getVillageCenterImagePath,
} from '@pillage-first/game-assets/resources-page-assets';
...
import styles from './resource-field-canvas.module.scss';
...
type ResourceFieldCanvasProps = {
  composition: ResourceFieldComposition;
  children: ReactElement;
};

const fieldPaths: Record<number, string> = {
...
export const ResourceFieldCanvas = ({
  composition,
  children,
}: ResourceFieldCanvasProps) => {
  const navigate = useNavigate();
  const { timeOfDay } = useContext(CookieContext);
  const tribe = useTribe();
  const [hoveredSlot, setHoveredSlot] = useState<number | null>(null);
  const [activeSlot, setActiveSlot] = useState<number | null>(null);
  const [isVillageCenterHovered, setIsVillageCenterHovered] = useState(false);

  const backgroundImage = getResourceFieldImagePath(composition, timeOfDay);
  const villageCenterImage = getVillageCenterImagePath(tribe, timeOfDay);

  const handlePathClick = (slotId: number) => {
    setActiveSlot(slotId);
    const link = document.querySelector(`[data-building-field-id="${slotId}"]`);
    if (link) {
  ...
  const handleVillageCenterClick = () => {
    navigate('../village');
  };

  return (
    <HoverContext.Provider value={{ hoveredSlot, setHoveredSlot }}>
      <div
      ...
      >
        ...
        >
          {Object.entries(fieldPaths).map(([slotId, path]) => (
            ...
            <path
              ...
              className={`
                ${styles['field-path']}
                ${hoveredSlot === Number(slotId) ? styles['field-hovered'] : ''}
                ${activeSlot === Number(slotId) ? styles['field-active'] : ''}
              `}
              onMouseEnter={() => setHoveredSlot(Number(slotId))}
              onMouseLeave={() => setHoveredSlot(null)}
              onClick={() => handlePathClick(Number(slotId))}
            />
          ))}
        </svg>
        <img
          src={villageCenterImage}
          alt="Village Center"
        ...
        >
          ...
          <path
            ...
            onMouseLeave={() => setIsVillageCenterHovered(false)}
            onClick={handleVillageCenterClick}
          />
        </svg>
