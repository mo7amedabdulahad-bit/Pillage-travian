...
const defaultFieldPaths = {
  ...
  18: 'M310 188.5c0 1.6.6 2.5 1.5 2.5.8 0 1.5.4 1.5 1 0 .5-.8 1-1.8 1-2.7 0-4.2 2-4.2 5.7 0 2.8.5 3.5 3.5 4.7 2.5 1.1 3.3 1.9 2.9 3-.4 1-.1 1.6.7 1.6.8 0-.5.9-2.8 2.1-3.5 1.7-4.3 2.6-4.3 4.9 0 1.6-1.1 4.2-2.5 5.9-2.4 2.7-2.6 3.8-2.6 12.8 0 5.4.2 12.6.5 16.1.5 6.2.5 6.4-2.4 8-1.7.9-3 2.2-3 2.9 0 .7-1.3 2.8-3 4.6-1.6 1.9-3 4-3 4.6 0 .6-1.1 1.8-2.5 2.7-4.1 2.7-3.1 4.6 4.3 8.3 6.7 3.3 6.9 3.4 19.6 3.2 7-.1 14.4.4 16.4 1 3.3.9 4.8.7 11.2-1.6 4.1-1.4 8.5-2.9 9.8-3.2 1.2-.3 2.2-.9 2.2-1.4 0-.5.6-.9 1.4-.9.8 0 1.6-.3 1.8-.8.2-.4 4.1-3.6 8.8-7.1 5.6-4.2 10.3-6.9 13.9-7.9 3.8-1.1 6.6-2.8 9.1-5.3 2-2.1 5.4-4.5 7.7-5.4 2.2-.9 4.1-2.3 4.2-3.3.5-4.5.2-6.4-1.4-7.6-2-1.4-1.7-3.4 1-5.8 1.6-1.4 1.7-1.8.5-1.9-.8-.1-1.9-.3-2.5-.4-.5-.2-1.7-.3-2.5-.4-4.3-.4-4.2-1.8.4-6.6 2.5-2.6 4.6-5 4.6-5.5 0-1.6-2.6-2.8-7.5-3.5-5.2-.8-16.7-6.3-15.9-7.7.3-.4-2.2-2.1-5.5-3.7-3.3-1.7-6.3-3.6-6.6-4.2-1.3-1.9-9.9-5-16.5-5.9-7.2-.9-15.9-3.1-26.6-6.6-9.1-3.1-10.4-3.1-10.4.1z',
};

const villages = [
  { id: 26041, rfc: '4446', name: 'Village 1 4-4-4-6' },
  { id: 25841, rfc: '3456', name: 'Village 2 3-5-4-6' },
  { id: 26042, rfc: '4347', name: 'Village 3 4-3-4-7' },
  { id: 25842, rfc: '3447', name: 'Village 4 3-4-4-7' },
  { id: 27438, rfc: '11115', name: 'Village 5 1-1-1-15' },
  { id: 19195, rfc: '3339', name: 'Village 6 3-3-3-9' },
  { id: 25640, rfc: '4536', name: 'Village 7 4-5-3-6' },
  { id: 25241, rfc: '3456', name: 'Village 8 3-4-5-6' },
  { id: 25644, rfc: '5346', name: 'Village 9 5-3-4-6' },
  { id: 26845, rfc: '3546', name: 'Village 10 4-4-3-7' },
  { id: 25837, rfc: '4356', name: 'Village 11 4-3-5-6' },
  { id: 27844, rfc: '5436', name: 'Village 12 5-4-3-6' },
];

// Only compare villages with new compositions we need
const targetVillages = villages.filter(v => 
  ['3456', '4437', '4536', '5346', '11115', '4356', '5436', '3339'].includes(v.rfc)
);
...
async function main() {
  ...
  for (const village of targetVillages) {
    console.log(`\nExtracting ${village.name} (RFC ${village.rfc})...`);
    await page.goto(`https://nor1.ttwars.com/dorf1.php?newdid=${village.id}`, { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('svg.resourceField', { timeout: 10000 });
    
    const extracted = await page.evaluate(() => {
      const svg = document.querySelector('svg.resourceField');
      if (!svg) return null;
      const paths = {};
      svg.querySelectorAll('path').forEach(p => {
        const classes = [...p.classList];
        const slotClass = classes.find(c => c.startsWith('buildingSlot'));
        if (slotClass && slotClass !== 'buildingSlot') {
          const num = parseInt(slotClass.replace('buildingSlot', ''));
          if (!isNaN(num)) paths[num] = p.getAttribute('d');
        }
      });
      return paths;
    });
    ...
    if (!extracted) {
      console.log(`  ERROR: No SVG found`);
      continue;
    }
    
    const diffs = {};
    const identical = [];
    for (const [slot, path] of Object.entries(extracted)) {
      const slotNum = parseInt(slot);
      if (defaultFieldPaths[slotNum] && path !== defaultFieldPaths[slotNum]) {
        diffs[slotNum] = path;
      } else if (defaultFieldPaths[slotNum] && path === defaultFieldPaths[slotNum]) {
        identical.push(slotNum);
      }
    }
    
    console.log(`  Total slots: ${Object.keys(extracted).length}`);
    console.log(`  Different from default: ${Object.keys(diffs).length} slots [${Object.keys(diffs).join(', ')}]`);
    console.log(`  Identical to default: ${identical.length} slots [${identical.join(', ')}]`);
    
    if (!results[village.rfc]) results[village.rfc] = {};
    results[village.rfc] = diffs;
  }
  
  console.log('\n\n=== RESULTS SUMMARY ===');
  for (const [rfc, diffs] of Object.entries(results)) {
    console.log(`\nRFC ${rfc}: ${Object.keys(diffs).length} slots differ from default`);
    for (const [slot, path] of Object.entries(diffs)) {
      console.log(`  Slot ${slot}: ${path.substring(0, 60)}...`);
    }
  }
  
  // Write results to file
  const fs = await import('fs');
  fs.writeFileSync('C:\\Users\\Mohamed\\OneDrive\\Desktop\\Pillage-First-Ask-Questions-Later-master\\extracted-data\\all-diffs.json', JSON.stringify(results, null, 2));
